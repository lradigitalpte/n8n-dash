...
export default function SidebarNav() {
  ...
  const isKnowledge = normalized === "/dashboard/knowledge";
  const isAiTest = normalized === "/dashboard/ai-test";
  ...
  return (
    <div className="flex shrink-0 items-center gap-2 border-b border-border/60 bg-background/40 px-3 py-2.5 dark:bg-transparent dark:border-border/40 md:px-4 md:py-3">
      <div className="flex items-center gap-1.5 rounded-md border border-border/50 bg-muted/30 px-1 py-0.5 dark:border-border/40 dark:bg-white/[0.04]">
        <Link href="/dashboard" className={linkClass(isInbox)}>
          Home
        </Link>
        <span className="h-3.5 w-px shrink-0 bg-border/70 dark:bg-white/15" aria-hidden />
        <Link href={"/dashboard/leads" as never} className={linkClass(isLeads)}>
          Leads
        </Link>
        <span className="h-3.5 w-px shrink-0 bg-border/70 dark:bg-white/15" aria-hidden />
        <Link href={"/dashboard/knowledge" as never} className={linkClass(isKnowledge)}>
          Knowledge
        </Link>
        <span className="h-3.5 w-px shrink-0 bg-border/70 dark:bg-white/15" aria-hidden />
        <Link href={"/dashboard/ai-test" as never} className={linkClass(isAiTest)}>
          AI Test
        </Link>
      </div>
